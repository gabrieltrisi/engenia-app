...
export function DeleteCompanyButton({ companyId }: DeleteCompanyButtonProps) {
  ...
  async function handleDelete() {
    try {
      ...
      toast.error(
        "Erro ao excluir empresa. Verifique se ela possui clientes ou projetos vinculados.",
      );
    } finally {
      setLoading(false);
    }
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-950 p-6 text-white shadow-xl">
            <h2 className="text-lg font-bold">Excluir empresa</h2>

            <p className="mt-2 text-sm text-zinc-400">
              Tem certeza que deseja excluir esta empresa? Se ela possuir
              clientes ou projetos vinculados, a exclusão pode falhar.
            </p>
